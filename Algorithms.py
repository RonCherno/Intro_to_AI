...
class AStarEpsilonAgent():
    ...
    def get_min_focal(self, open: heapdict, epsilon: int) -> Tuple[Tuple[int, bool, bool], Node]:
        focal = heapdict.heapdict()
        curr_min = open.popitem()           #guess open isnt empty
    ...
    def search(self, env: DragonBallEnv, epsilon: int) -> Tuple[List[int], float, int]:
        ...
        while (open):
            curr_state, curr_node = self.get_min_focal(open, epsilon)
            close[curr_state] = curr_node
            expanded = expanded+1
            if (self.env.is_final_state(curr_state)):
                return (curr_node.c_actions, curr_node.c_total_cost, expanded-1)
